"""
GNI Keyword Intelligence Page Builder - Clean rewrite
Key lesson: NEVER use f-strings for lines containing TSX {expressions}
Use actual Unicode emoji characters directly in regular strings
GNI-R-046, GNI-R-062
Run from: C:\HDCS_Project\03\GNI_Autonomous
Usage: python build_keyword_intelligence_page.py
"""
...
# ── FILE 1: API ROUTE ────────────────────────────────────────────────────────
api_dir = os.path.join("src", "app", "api", "keyword-intelligence")
...
with open(os.path.join(api_dir, "route.ts"), "w", encoding="utf-8", newline="\n") as f:
    f.write("\n".join(api_lines) + "\n")
print("OK Written: src/app/api/keyword-intelligence/route.ts")

# ── FILE 2: PAGE ─────────────────────────────────────────────────────────────
page_dir = os.path.join("src", "app", "keyword-intelligence")
os.makedirs(page_dir, exist_ok=True)

# All emoji as actual Unicode characters - NO f-strings - NO \U escapes in strings
LOCK    = "\U0001f510"
SHIELD  = "\U0001f6e1"
BALLOT  = "\U0001f5f3"
BULB    = "\U0001f4a1"
...
"            <div>",
"              <h1 className='text-2xl font-bold text-white'>" + LOCK + " Keyword Intelligence Engine</h1>",
...
"            </div>",
"            <a href='/' className='text-sm text-blue-400 hover:text-blue-300 shrink-0 mt-1'>" + ARROW + " Dashboard</a>",
"          </div>",
"          {stats && (",
...
"                <div className='text-xs text-red-600'>" + CROSS + " Auto-Rejected</div>",
"              </div>",
...
"                <div className='text-xs text-green-600'>" + CHECK + " Recommended</div>",
"              </div>",
...
"                <div className='text-xs text-orange-600'>" + SHIELD + " Security Veto</div>",
"              </div>",
"            </div>",
"          )}",
"        </div>",
...
"        {loading && <div className='text-center py-20 text-gray-400'><div className='text-4xl mb-4'>" + HOURGLASS + "</div><p>Loading...</p></div>}",
"        {!loading && (",
"          <>",
"            <div className='flex gap-2 mb-6 flex-wrap'>",
"              {([",
"                { key: 'journey',   label: '" + COMPASS + " Journey',  count: logs.length },",
"                { key: 'pending',   label: '" + HOURGLASS + " Pending', count: pending.length },",
"                { key: 'approved',  label: '" + CHECK + " Approved',   count: approved.length },",
"                { key: 'explainer', label: '" + BULB + " How It Works', count: 0 },",
...
"                    <div className='text-xs text-gray-500 uppercase tracking-wider mb-3'>" + SHIELD + " Security Layer Stats</div>",
...
"                      ))}",
"                    </div>",
"                    <div className='text-xs text-gray-500 uppercase tracking-wider mb-3'>" + BALLOT + " Agent Votes</div>",
...
"                            <span className={`text-xs font-bold ${decisionColor(log.final_decision)}`}>{log.final_decision === 'RECOMMEND' ? '" + CHECK + " RECOMMEND' : log.final_decision === 'AUTO_REJECT' ? '" + CROSS + " AUTO-REJECT' : log.final_decision}</span>",
"                            {log.security_veto && <span className='text-xs bg-red-900 text-red-300 px-2 py-0.5 rounded-full border border-red-600 font-bold'>" + SHIELD + " VETO</span>}",
"                          </div>",
"                          <span className='text-gray-600 text-xs'>{expanded === log.id ? '" + BACK + "' : '" + FWD + "'}</span>",
"                        </div>",
...
"                          {log.vote_result && (",
"                            <div>",
"                              <div className='text-xs text-gray-500 uppercase tracking-wider mb-2'>" + BALLOT + " Agent Votes ({log.vote_result})</div>",
"                              <div className='grid grid-cols-3 gap-3'>",
"                                <AgentBadge vote={log.agent_analyst_vote}  confidence={log.agent_analyst_confidence}  name='" + ANALYST + " Analyst' />",
"                                <AgentBadge vote={log.agent_auditor_vote}  confidence={log.agent_auditor_confidence}  name='" + SHIELD  + " Auditor' />",
"                                <AgentBadge vote={log.agent_advocate_vote} confidence={log.agent_advocate_confidence} name='" + SCALES  + " Advocate' />",
"                              </div>",
"                              <div className='mt-2 space-y-1 text-xs text-gray-500'>",
"                                {log.agent_analyst_reason  && <div>" + ANALYST + " {log.agent_analyst_reason}</div>}",
"                                {log.agent_auditor_reason  && <div>" + SHIELD  + " {log.agent_auditor_reason} (atk: {Math.round(log.agent_auditor_attack_prob * 100)}%)</div>}",
"                                {log.agent_advocate_reason && <div>" + SCALES  + " {log.agent_advocate_reason}</div>}",
"                              </div>",
"                            </div>",
"                          )}",
"                          {log.sources?.length > 0 && (",
"                            <div>",
"                              <div className='text-xs text-gray-500 uppercase tracking-wider mb-1'>" + ANTENNA + " Sources ({log.sources.length})</div>",
...
"                {pending.length === 0 && <div className='text-center py-20 text-gray-500'><div className='text-4xl mb-4'>" + HOURGLASS + "</div><p>No keywords pending review.</p></div>}",
"                <div className='space-y-3'>",
"                  {pending.map(kw => (",
...
"                      </div>",
"                      <div className='mt-2 text-xs text-yellow-600'>" + WARN + " Approve or reject via Telegram.</div>",
"                    </div>",
"                  ))}",
"                </div>",
"              </section>",
...
"                {approved.length === 0 && <div className='text-center py-20 text-gray-500'><div className='text-4xl mb-4'>" + CHECK + "</div><p>No approved keywords yet.</p></div>}",
...
"                  <div className='text-xs text-gray-500 uppercase tracking-wider mb-4'>" + BULB + " 7 Security Layers</div>",
"                  <div className='space-y-3'>",
"                    {[",
...
"                </div>",
"                <div className='bg-gray-900 border border-gray-700 rounded-xl p-6'>",
"                  <div className='text-xs text-gray-500 uppercase tracking-wider mb-4'>" + BALLOT + " 3-Agent MAD Voting</div>",
"                  <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mb-4'>",
"                    {[",
"                      { name: '" + ANALYST + " Intelligence Analyst', role: 'Relevance scorer', q: 'Is this a genuine intelligence signal?', ok: 'Conflict/sanctions/tech warfare signal', no: 'Generic word, no intelligence value' },",
"                      { name: '" + SHIELD  + " Security Auditor',     role: 'Attack detector (VETO)', q: 'Could this be a manipulation attempt?', ok: 'Organic rise, diverse trusted sources', no: 'Sudden spike, low source diversity' },",
"                      { name: '" + SCALES  + " Devils Advocate',      role: 'Novelty validator', q: 'Is this truly new or just noise?', ok: 'Not in 90-day history, 5x+ spike', no: 'Fades in 48hrs, single news cycle' },",
...
"                        <div className='text-xs text-green-400 mb-1'>" + TICK + " {ok}</div>",
"                        <div className='text-xs text-red-400'>" + XTICK + " {no}</div>",
"                      </div>",
"                    ))}",
"                  </div>",
...
"                    ))}",
"                  </div>",
"                  <div className='text-xs text-red-400 bg-red-950 border border-red-800 rounded-lg p-3'>" + SHIELD + " Security Veto: If Auditor detects attack probability &gt; 70%, auto-rejected regardless of other votes. (GNI-R-103)</div>",
"                </div>",
"              </section>",
"            )}",
"          </>",
...
with open(os.path.join(page_dir, "page.tsx"), "w", encoding="utf-8", newline="\n") as f:
    f.write("\n".join(page_lines) + "\n")
print("OK Written: src/app/keyword-intelligence/page.tsx")
