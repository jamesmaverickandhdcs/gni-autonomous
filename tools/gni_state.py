...
def norm_md5(raw: bytes) -> str:
    """R-S98-3: hash what the content IS, not how git handed it over. A BOM or a
    CRLF is platform noise, not a change in the workflows this section describes."""
...
def render(wfs: list[dict], stored: list[str], stored_note: str, head: str,
    ...
    o.append("")
    o.append(f"**GENERATED by `tools/gni_state.py` from HEAD `{head}` \u2014 "
             f"{len(wfs)} workflow files, manifest md5 `{manifest}` "
             "(EOL-normalised, R-S98-3). This section carries NO CLOCK: re-running it "
             "on an unchanged tree reproduces these bytes exactly. "
...
def main() -> int:
    ...
    ap.add_argument("--session", type=int, required=True,
                    help="session number; names the OUTPUT document. No default on "
                         "purpose: a bare invocation used to write S98 content into "
                         "GNI_ARCHITECTURE_S94.md (item 5.34).")
    ...
    if not WF_DIR.is_dir():
        ...
        return 3
    wf_paths = sorted(WF_DIR.glob("*.yml"))
