          ...
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="text-right text-sm text-gray-400">
              ...
              <div>Pipeline: <span className="inline-flex items-center gap-1">
                {latestRun?.run_at === undefined || latestRun?.run_at === null ? (
                  <><span className="inline-block w-2 h-2 rounded-full bg-gray-500"></span><span className="text-gray-400">Unknown</span></>
                ) : (Date.now() - new Date(latestRun.run_at).getTime()) / 3600000 < 14 ? (
                  <><span className="inline-block w-2 h-2 rounded-full bg-green-400"></span><span className="text-green-400">Active</span></>
                ) : (Date.now() - new Date(latestRun.run_at).getTime()) / 3600000 < 26 ? (
                  <><span className="inline-block w-2 h-2 rounded-full bg-yellow-400"></span><span className="text-yellow-400">Delayed</span></>
                ) : (
