                ...
                <div className="bg-green-950 border border-green-800 rounded-xl p-4 mb-4 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-green-300 text-xs mt-2 leading-relaxed opacity-80">
                      {latest.sentiment?.toUpperCase() === latest.mad_verdict?.toUpperCase() ? <>The AI report and the MAD debate agents independently reached the same {latest.sentiment?.toLowerCase()} conclusion.</> : <>The AI report reached {latest.sentiment?.toLowerCase()} while the MAD debate settled on {latest.mad_verdict?.toLowerCase()} &mdash; compatible, non-contradicting signals.</>}{latest.sentiment?.toUpperCase() === latest.mad_verdict?.toUpperCase() ? <> When both systems agree, it means the directional signal is robust — the same story is visible from both a news-analysis perspective and a future-threat-modelling perspective. This is the highest-confidence signal GNI can produce.</> : <> A neutral debate verdict beside a directional report means the agents found no opposing case &mdash; the report&apos;s direction stands unchallenged rather than independently confirmed.</>}
                    </div>
                  </div>
                </div>
              )}
