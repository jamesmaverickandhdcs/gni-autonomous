...
**GENERATION 14 - 2026-09-02 (S94 close). SUPERSEDES generation 13 (`GNI_TARGET_AND_ORDER_S93.md`).**
Regenerated, never appended. The LIVE order is the HIGHEST session number.

---

## NEXT SESSION'S MISSION (S95)

**Classify all 134 registered rules CHECKABLE / NOT CHECKABLE, and make the top five
executable.**

WHY THIS IS TOP. It is JAMES'S roadmap (DECISION S93-2), and ARCHITECTURE 8.3 states the
mechanism plainly: GNI holds 134 rules, all prose, all requiring an LLM to read and apply,
none executable - yet many are mechanically checkable. *Every judgment converted into a
check is an agent no longer needed.* S93 built the detector; S94 built the world model;
S95 starts moving law out of prose and into code.

**Definition of done:** every rule in `GNI_RULES_S95.md` carries a CHECKABLE marker with a
one-line reason, the counts are stated, and FIVE rules exist as executable checks that a
workflow can run. S94's four new rules already carry their markers - use them as the seed
and the format, not as an exception.

**Discriminating cert (R-S90-1):** each of the five checks must FAIL on a deliberately
violating fixture and PASS on the real tree. A check that passes on both has certified
nothing. Build them against a FIXTURE first (R-S94-4) - S94's fixture caught four generator
bugs before the real tree saw one of them.

**Ordering note:** items 5.20 and 5.21 are S95-shaped and cheap. 5.20 in particular is the
detector's own blind spot: `tools/gni_state.py` sits OUTSIDE the CI glob, so its seven
control probes only run when a human runs them.

**A candidate for the fifth check, free:** 9.17's claim - "N scheduled workflows" - is now
generated by `tools/gni_state.py`. A check that greps the live docs for a workflow count and
compares it to the generator's is three lines and closes a whole finding shape.
...
**S94 (generation 14). Produced by BYTE COPY of generation 13 plus anchored patches
...
- MISSION replaced: S94's section-7 generator (done, `44a3cba`) -> S95's rule-to-check pass.
- THE ORDER: a CLOSED AT S94 block added at the top; the S93 and S92 blocks below it keep
  their own session labels. ONE correction was written INTO the S93 block, in brackets and
  attributed - "six code files" is six SITES in FOUR files. A wrong number left standing in
  a live file is the rot this whole document exists to prevent; erasing it unattributed
  would be worse.
- **6.9 amended rather than re-minted** - its subject is unchanged and its figures are now
  generated. Two sub-findings folded into it instead of taking new numbers.
- GRAVEYARD: unchanged at SEVEN rows, carried by byte copy - not retyped, not re-read.
- **RULINGS THIS SESSION: NONE.** James set no new direction; S94 executed the mission
  DECISION S93-2 had already assigned. There is therefore no DECISION S94-n line, and the
  absence is stated rather than left to be inferred.
- One judgement was made without a ruling and is recorded as mine: the S94 commit message
  lost its blank line after the subject, and the commit was already pushed. Rewriting shared
  history to fix formatting was judged not worth it. Not amended.
- ARCHITECTURE: section 7 is GENERATED and now populated; the ROADMAP table's S94 row
  carries the commit; section 11 D1 gains this generation's rho. Sections 5 and 6 remain
  EMPTY and that is the correct state, not an omission.
- RULES -> four earned, `R-S94-1` .. `R-S94-4`, each carrying a **CHECKABLE** marker. The
  markers are S95's mission format, seeded one session early on purpose.
- CONTRACT: UNCHANGED at v9, byte-identical, md5 `d7e68e815a17eaffbaedc5d6b4494bde`
  (DECISION S89-6). Protocol: UNCHANGED at v11. No rule of engagement changed this session,
  and the law-vs-state test says that is the healthy outcome.

**CLOSED:** the S94 MISSION (`tools/gni_state.py`, `44a3cba`, discriminating cert passed,
control probe proven to fail loudly) · TWO banked regression figures corrected against the
generator · the three unread MAD debates (READ, not counted) - which turned ROOT 1.3 from a
two-sample ruling into a five-debate distribution.

**NEW ITEMS: FOUR. rho = 4/1 = 4.00 this generation, and it is not being hidden.**
**9.17** (a workflow count that was incremented, never measured) · **6.10** (the scheduler's
real lateness band, +4h07m to +6h58m over n=9) · **5.20** (the generator sits outside CI's
glob, so its own probes never run unattended) · **5.21** (no `.gitattributes`, so a fresh
checkout can fake a whole-file diff in the generated document).
**Findings routed OUT of this file this session: NONE.** Everything measured at S94 is
numbered here.

**ONE ITEM WAS CONSIDERED AND DELIBERATELY NOT MINTED:** the `gni_ci_harness` package waste
and `gni_selfcheck`'s absent `pip install` are folded into 6.9. Both are the same subject as
an open item, and minting numbers for sub-findings inflates the queue that D1 measures.
