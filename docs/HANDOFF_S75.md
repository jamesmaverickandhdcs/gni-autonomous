# HANDOFF S75 -> S76
DATE: 2026-07-18 | HEAD: `577b26d` + close commit (verify ls-remote) | MODEL: Fable 5
Read ONCE. Standing rules: docs/GNI_RULES.md by ID (current through R-S74-3 IN BOOK; R-S75-1/2/3 pending below).
Contract: docs/CONTRACT.md (unchanged). 5 commits incl. close.
...
| GPVS-NUMBERS | 3 pages = 3 objects: timeline entries (50) / verified_at rows (294, accurate=52) / outcome direction_correct 3d-7d (38%/42%) | greps |
...
## 5. TRAPS (<=8 lines)
- Historical pipeline_runs rows keep INFLATED total_after_dedup -- honest fossils, never rewrite;
  only runs after `8eed829` go monotonic. Old transparency funnels still show 360-style numbers.
- Funnel's default-True stage flags with "Not evaluated" reasons are DELIBERATE design;
  the fix lives in main.py counting only. Forensic trace (line 551) already conjoins correctly.
- gni_graph_job triple .eq filter is CORRECT -- do not re-flag from truncated grep output (R-S66-1 kin).
...
HEAD = `577b26d`+close commit TREE CLEAN -- 7PLUS-TILE certified (111), TRANS-COUNT shipped prod-pending, DET-DEAD executed, CERT-BATCH-2 5/5, 81-claim TRUE
...
FIRST MOVE = ls-remote + git status; then TRANS-COUNT-CERT is cheapest read if a cron has fired since `8eed829`
...
GPVS objects: history reads outcomesData.timeline; predictions reads verified_at+accurate; patterns reads outcomes direction_correct_3d/7d.
...
R-S75-3: When a sequential funnel persists per-stage flags with default-True "not evaluated"
  semantics, every aggregate count over the trace must conjoin ALL prior stage flags --
...
out proved the site's boldest claim -- 81 patterns -- was true all along. Two of my own leads
died under full reads, and that's the system working: the same discipline that catches the
code's lies catches mine. Five screenshots certified five pages. The ghost lineage got no
new night to walk. Four commits, two retractions, zero fossils shipped. 👊
