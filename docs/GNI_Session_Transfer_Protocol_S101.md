# GNI SESSION TRANSFER PROTOCOL v16
...
## 1. STATE (<=10 lines)
L1 Pipeline: {one line}  L2 MAD: {N debate + M watch, split at JOB level}  L3 GPVS: {one line}
...
4a. ROADMAP CHECK (v12). Is any declared roadmap or arc COMPLETE at this close? If YES,
   CONTRACT's PHASE TRANSITION runs BEFORE the order is regenerated: declare ACHIEVED with
   evidence (the commits and runs), archive the finished arc, JAMES declares the next target
   or roadmap TOGETHER WITH ITS WRITTEN COMPLETION TEST (CONTRACT v10), and only then is the
   order regenerated with every surviving item RE-CLASSIFIED and nothing inherited. If NO,
   write "roadmap N of M" in one line and move on. SILENCE IS NOT AN ANSWER: an unwritten
   check cannot be audited, and the arc that dies is the one no close ever asked about (D4).
...
9a. REGENERATE EVERY GENERATED ARTIFACT THAT READS A FILE THIS CLOSE JUST CHANGED (v13).
    Step 9 appends to the register, which moves both its marker count and its md5. Item 5.26
    (C6, shipped S98) turns `rule_checks` RED when `docs/GNI_MACRO_MAP_S{N}.md` is stale
    against the live register, so the close's OWN docs commit is now able to redden the
    detector the session just made green. After the rules land and BEFORE the commit, run:
      `python tools/gni_macro_map.py --session {N}`
      `python tools/gni_rule_checks.py .`      (expect 7 checked, 0 failed)

    STAGE A NEW OR CHANGED TOOL BEFORE REGENERATING ANYTHING (v15, R-S100-2). The
    section-5 generator inventories `git ls-files '*.py'`. An UNTRACKED tool is not in
    that list, so the section it produces omits it and is stale at the commit that
    introduces it. S100 hit this twice in one session. The order is not optional:
      `git add tools/<new_or_changed>.py`
      `python tools/gni_blocks.py --session {N} --src docs/GNI_ARCHITECTURE_S{N}.md`
      `python tools/gni_state.py  --session {N} --src docs/GNI_ARCHITECTURE_S{N}.md`
      `python tools/gni_macro_map.py --session {N}`
      `python tools/gni_rule_checks.py .`
    Then run the two section generators a SECOND time and compare the document's
    EOL-normalised md5 with the first: roadmap 2's completion test is byte-identity on an
    unchanged tree, and this is the only place in the close that actually tests it.
    Both `gni_blocks.py` and `gni_state.py` now REQUIRE `--session`; neither has a default.
      `python tools/gni_rule_checks_fixture.py` (expect 0 mismatches)
    A close that ships a register without regenerating its map has broken the build with
    paperwork. This step is the first one whose omission the repo can detect by itself.

    THIS GATE ALSO RUNS AT THE MISSION COMMIT, NOT ONLY HERE (v16). A tool changes when
    the mission ships, which is hours before this step. S101 added a seventh check to
    `gni_rule_checks.py`, ran the detector, saw 7 of 7 green, committed and pushed - and
    CI went red on `gni_rule_checks_fixture.py`, which THIS STEP ALREADY NAMES. The step
    was not wrong; it was scheduled after the commit it needed to gate. Before ANY commit
    that touches a file under `tools/`, run the fixture as well as the detector.

    A CHECK AND ITS FIXTURE FAMILY ARE ONE ARTIFACT (v16, from the same failure). A check
    added to `CHECKS` without a family in `EXPECT` does not fail alone: the synthetic
    repos carry only what the existing checks need, so the new check raises INSTRUMENT
    ERROR, `main()` returns 2 at the first one, and EVERY family reports the same code -
    eleven of fourteen mismatched at once, with nothing in the output naming the cause.
    A new check ships with (i) whatever inputs its synthetic repo now needs and (ii) at
    least one family that makes it go RED. A check the fixture cannot fail has no control
    probe, which is R-S93-1 with the halves swapped.
...
4. COUNT WHAT RAN WHILE NOBODY WAS WATCHING. List MAD runs since the handoff's date and say
   ...
   the presence of `ARB-FIT`, never by time alone. AMENDED S99: separate them at JOB level
   instead - `gh run view <id> --json jobs` reads `run-mad success` with `grounding-watch
   skipped` for the debate and the reverse for the watch, in one call and with no log
   download. `ARB-FIT` stays the fallback when job data is absent. A `skipped` job reports
   `completedAt` BEFORE `startedAt`, so exclude skipped jobs from duration arithmetic or it
   computes a negative (R-S84-4, amended S99).
...
- **v16 - S101 (2026-09-05).** PART C step 9a EXTENDED, nothing renumbered. Three things,
  all from one failure at this close. First, the step's own expected detector count still
  said six after a seventh check shipped - the step written to catch staleness had gone
  stale. Second, the step is a CLOSE step and a tool changes at the MISSION commit: S101
  added C7, ran the detector, saw seven green, pushed, and CI went red on the fixture the
  step already names, one commit too late. The gate now runs wherever `tools/` is touched.
  Third, a check added to `CHECKS` with no family in `EXPECT` does not fail alone - it
  raises INSTRUMENT ERROR in every synthetic repo and eleven of fourteen families mismatch
  at once, naming nothing. A check and its fixture family are one artifact. PART A, PART B
  and PART D are byte-identical to v15. No CONTRACT bump: this changes when a gate runs,
  not who may do what.
- **v15 - S100 (2026-09-04).** PART C step 9a EXTENDED, nothing renumbered. Step 9a already
  said to regenerate what the close made stale; it assumed the only such input was the
  register. S100 shipped `tools/gni_blocks.py`, which reads the tracked file list, and an
  untracked generator is invisible to its own output - the section was stale at its own
  commit, twice. The step now names the staging that must precede regeneration and the
  second render that proves byte-identity. PART A, PART B and PART D are byte-identical
  to v14.
- **v14 - S99 (2026-09-04).** PART D step 4 only. R-S84-4 said to separate the MAD debate from the
  grounding-watch by the presence of `ARB-FIT`, which requires downloading a run log; job-level data
  separates them in one call and was measured on three runs at S99. The rule was amended in
  `GNI_RULES`, and R-S82-4's sibling sweep requires the TEMPLATE that encodes the old method to be
  swept with it - a template left unswept restores the old rule by itself, which is how v1 kept the
  queue alive for three weeks after CONTRACT v4 moved it. PART C is byte-identical to v13.
- v13 - S98 (2026-09-04): ONE TEMPLATE STEP ADDED - Part C gains step 9a, REGENERATE GENERATED
  ARTIFACTS, immediately after step 9 appends the rules. Steps were NOT renumbered (DECISION
  S96-4's standing constraint: CONTRACT and this file both cite "PART C step 5" and "step 13"
  by number). Cause: item 5.26 shipped at S98 makes a stale macro map a RED build, and the
  close itself is the event most likely to make it stale - the register grows in step 9 and
  the map was generated before it. The first close-procedure step that the repository can
  detect the omission of on its own.
- v12 - S96 (2026-09-03): ONE TEMPLATE STEP ADDED - Part C gains step 4a, ROADMAP CHECK, before
  the order is regenerated. Steps were NOT renumbered: CONTRACT v9 and this file both cite
  "PART C step 5" and "step 13" by number, so renumbering would silently break two live
  citations. The sweep that produced this ran first and found NOTHING: `grep -n 'PHASE
  TRANSITION\|roadmap\|ROADMAP'` against the v11 bytes returned zero hits, so the close
  procedure carried no instruction for a finishing arc at all. The empty result IS the record
  of the sweep (R-S82-4) and was also the finding: what CONTRACT required, the close never
  asked for. S96 closed the last row of a four-session roadmap and would have regenerated the
  order without declaring anything, exactly as D4 did for five months, had James not asked.
