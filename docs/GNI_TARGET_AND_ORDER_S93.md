...
**GENERATION 13 - 2026-09-01 (S93 close). SUPERSEDES generation 12 (`GNI_TARGET_AND_ORDER_S92.md`).**
Regenerated, never appended. The LIVE order is the HIGHEST session number.

---

## NEXT SESSION'S MISSION (S94)

**`tools/gni_state.py` - the generator for ARCHITECTURE section 7 ONLY.**
Sections 5 and 6 are NOT in scope. Section 7's inputs are already bounded and readable:
`.github/workflows/*.yml`, `gh secret list`, and `git grep`. Section 5 needs AST work and
would make the generator grow in its first outing.

WHY THIS IS TOP. It is the next brick after S93's detector, and every hand-measured
inventory in this project has rotted. ARCHITECTURE 8.2: a system cannot repair what it
cannot describe, so 5/6/7 are Layer 2's prerequisite, not a detour from it.

**Definition of done:** `python tools/gni_state.py` writes ARCHITECTURE section 7 in
`docs/GNI_ARCHITECTURE_S94.md` and exits 0, listing every workflow, its cron, the secrets
it reads, its entrypoint, and - the part no inventory has had - `secret -> workflow ->
code consumer` as ONE chain. A generator that stops at "which workflow reads this secret"
reproduces exactly the trap S93 spent a block undoing: `GROQ_MODEL_FALLBACK` is read by no
workflow AND by six code files.

**Discriminating cert (R-S90-1):** change one secret line in one workflow, re-run, and the
output must CHANGE. If it does not, the generator is not reading what it claims to read.
The FIRST output must also agree with what S92 and S93 measured by hand, used as a
regression test: `GROQ_MODEL_FALLBACK` = 6 code files / 0 workflows · `GROQ_TEST_ONLY` and
`TELEGRAM_CHAT_ID` = 0 consumers each · `gni_ci_harness.yml` = 0 secrets. A disagreement
is a finding either way and must be reported, not reconciled silently.

**Ordering note:** if S94 fixes 5.14, item 5.17 closes with it and no allowlist is ever
built (DECISION S93-1). If S94 does not, 5.17's clock starts.
...
- **TRAP RETIRED, not carried.** The three "stored but read by no workflow" secrets were
  traced: `GROQ_MODEL_FALLBACK` is read by SIX code files (`llm_health_probe:17`,
  `nexus_analyzer:29`, `mad_protocol:52`, `intelligence_funnel:1070`, plus two audit
  sites) and is item **1.12**, open since S85 - the trap was 1.12 under a new name.
...
- **6.9** **NEW (S93) [MEASURED].** There is NO dependency manifest - no
  `requirements.txt`, no `pyproject.toml`, nothing. Each of the 8 workflows carries its own
  inline `pip install` list, and `gni_mad.yml` alone carries TWO different lists in its two
  jobs (6 packages vs 3). Eight sources of truth for one codebase, none diffable. Direct
  input to ARCHITECTURE section 7 and to the S94 generator.
...
**S93 (generation 13). Produced by BYTE COPY of generation 12 plus anchored patches
(DECISION S92-5). The whole of this section is replaced, never appended to - generation 12
carried generation 11's CLOSED/NEW lines unlabelled, and two consecutive readers took them
for its own.**
- MISSION replaced: S93's CI harness job (done) -> S94's section-7 generator.
- THE ORDER: a CLOSED AT S93 block added at the top; the S92 block below it is unchanged
  and remains labelled with its own session.
- ROADMAP: S93's row in `GNI_ARCHITECTURE_S93.md` now carries the commit and the cert run.
  The roadmap is stated in THREE places on purpose - the LOAD CHECK, this section, and the
  ARCHITECTURE table - so no session can read its mission without seeing the plan it serves.
- TRAPS: the three-secret trap is RETIRED after tracing, not carried a second time.
- GRAVEYARD: unchanged at SEVEN rows, carried by byte copy - not retyped, not re-read.
- DECISION S93-1 (James): the expected-fail allowlist is NOT built. Item 5.17 is filed and
  bound to 5.14 - fix the harnesses and the item closes; defer them and the clock starts.
- DECISION S93-2 (James): the S93-S96 roadmap is JAMES'S, not a prior session's suggestion,
  and must be visible at every session open. Hence the three homes above.
- ARCHITECTURE section 11 D1: rho MEASURED across generations and given a SCOPE - it
  measures queue-working sessions and cannot measure direction-setting ones.
- Protocol -> v11: Part B's STATE line now reads `L2 MAD: {N debate + M watch, by ARB-FIT}`.
- CONTRACT: UNCHANGED at v9, byte-identical, verified by md5sum (DECISION S89-6). No law
  changed this session, and the law-vs-state test says that is the healthy outcome.

**CLOSED:** the S93 MISSION (CI harness job, `944c4f0`, certified by run `33529254247`) ·
the three-secret TRAP (all three traced to a consumer or to a deliberate absence; **nothing
deleted**) · the two unread MAD debates (READ, not counted).

**NEW ITEMS: NINE. rho = 9/1 this session and it is not being hidden.** **5.15** (36
assertions never execute) · **5.16** (the 42 selftests cannot be answered by CI) · **5.17**
(a permanently-red detector) · **5.18** (wrongness ledgers are never re-read) · **5.19**
(R-S91-4's specimen disproven) · **6.9** (no dependency manifest; 8 inline lists) ·
**7.4** (grounding-shadow counts vary ~9x, n=2) · **9.15** (dead model as innermost default)
· **9.16** (a published sentence claims more than 6.7 measured).
**Findings routed OUT of this file this session: NONE.** Where a session routes findings to
ARCHITECTURE or the RULES, they must still be named here with a pointer, or the queue-side
count silently under-reports - which is what happened to S92's D1-D7.
