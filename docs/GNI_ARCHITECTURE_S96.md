...
| S96 | ✅ **DONE** — `tools/gni_macro_map.py` + `docs/GNI_MACRO_MAP_S95.md`, commit `09588b2`; discriminating cert passed, three negative controls exit 2 | measures the gap instead of estimating it |
...
|---|---|---|
| S98 | 5.14 + 5.17 + 9.15 + C6 map-staleness guard + M4 generator naming | the detector goes GREEN, so RED carries information again |
