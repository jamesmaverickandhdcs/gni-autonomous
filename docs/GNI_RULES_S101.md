...
## RECOVERED AT S101 - the heartbeat and adaptive cadence rules (item 5.37)

Four March-2026 ids, recovered VERBATIM from four independent session records and then
confirmed against the tree: each is cited BY ID inside `ai_engine/monitoring_pipeline.py`,
which is why the text could be MATCHED rather than inferred. The manifest row for
`GNI-R-122` glossed it as "heartbeat standdown is correct behaviour"; that gloss was an
inference and it was wrong. It is retired with the row.

- **GNI-R-114** - The heartbeat pipeline uses ZERO Groq calls. Never add a Groq call to
  `gni_heartbeat.yml`. If the heartbeat spends quota, quota exhaustion becomes a
  monitoring blind spot. Confirmed at `ai_engine/monitoring_pipeline.py:4,7,313,466`.
  **CHECKABLE: yes** - a script can assert the watcher module makes no Groq call
- **GNI-R-115** - The adaptive pipeline adjusts its interval by escalation level:
  CRITICAL 0.5h and 0 Groq, HIGH 2h and 4 calls, ELEVATED 4h and 4, MODERATE 6h and 4,
  LOW 12h and 19. A fixed frequency wastes budget at LOW and misses signal at CRITICAL.
  **CHECKABLE: no** - the interval table has not been located in the tree, and no
  committed artifact publishes it
- **GNI-R-116** - The heartbeat triggers the adaptive pipeline when the escalation delta
  reaches 2.0. Filter `escalation_score > 0` in the heartbeat query: duplicate-skipped
  reports scoring 0.0 produced a false delta of 10. Confirmed at
  `ai_engine/monitoring_pipeline.py:8,49-50,167,424`.
  **CHECKABLE: yes** - `DELTA_THRESHOLD` is an AST-readable module constant
- **GNI-R-122** - Protection windows are 23:00-01:30 UTC and 09:00-10:45 UTC. No
  non-sacred pipeline runs inside them, so the 02:00 and 10:00 sacred runs are not
  starved of tokens by an adaptive or manual run. Confirmed at
  `ai_engine/monitoring_pipeline.py:35-39`.
  **CHECKABLE: yes** - `PROTECTION_WINDOWS` is AST-readable, and `C7` already reads it
  **INSTANCE (S101):** the stated rationale is token collision, and `GNI-R-114` says the
  heartbeat spends no tokens. The window is applied to the heartbeat anyway at
  `monitoring_pipeline.py:317-320`, which suppresses a zero-Groq check and returns
  `success`. A rule applied outside its subject. See ARCHITECTURE section 10 SLO-1 and
  order item 6.8.
...
`GNI-R-116` were registered in the same pass and so never needed a row. A row leaves
this table only when the ID is registered in PART 0 or PART 3 - `registered_ids` in
`tools/gni_rule_checks.py` skips only PART 1 and PART 2, being an index and a cluster
map - or when the citation is deleted.
...
## S100 ADDITIONS (2026-09-04)

- R-S100-1 - A CONTROL PROBE BUILT FROM SOURCES THE AUTHOR INVENTED TESTS THE AUTHOR'S
  ASSUMPTIONS, NOT THE SYSTEM. `tools/gni_blocks.py` passed 22 of 22 self-written probes and
  then reported 3 internal import edges across 79 modules and zero test coverage. Both figures
  were impossible. The fixture used `from ai_engine.analysis.mad_protocol import ...`, a line
  the repository has never contained; 29 modules push their own directory onto `sys.path`, so
  the real form is `from analysis.mad_protocol import ...` and the same file also answers to a
  bare `mad_protocol`. The probe agreed with the resolver because one author wrote both. A
  probe's fixtures must be COPIED FROM THE REAL TREE'S BYTES - grep the shapes out first - and
  the file must say where they came from. Rebuilt that way, the same tool found 84 edges and
  reproduced a five-edge module exactly against a verbatim grep it had never seen.
  **CHECKABLE: yes** - assert no probe fixture in `tools/` contains an import form that appears
  nowhere in the tracked tree.

- R-S100-2 - A GENERATOR THAT READS THE TRACKED TREE IS INVISIBLE TO ITSELF UNTIL IT IS STAGED.
  `gni_blocks.py` inventories `git ls-files '*.py'`. An untracked new tool is not in that list,
  so the section it generates omits it and is STALE at the very commit that introduces it. This
  bit twice in one session: once for the tool itself, once for a patch script sitting in the
  repo root. Order of operations at any close that ships or changes a tool: `git add` the tool
  FIRST, then regenerate every generated section, then run the detector, then commit.
  **CHECKABLE: yes** - assert the file count stamped in ARCHITECTURE section 5 equals
  `git ls-files '*.py' | wc -l` on the committed tree.

- R-S100-3 - A FIX IS NOT SHIPPED UNTIL THE RECIPIENT HAS VERIFIED THE BYTES. A corrected tool
  was written, certified and copied to the delivery directory, and the operator went on running
  the previous version for two exchanges, because no new download was issued and the old one
  was still being served. Nothing in the output said so; the tool ran, exited 0, and produced
  stale numbers. Every delivered artefact carries its EOL-normalised md5 and its byte count, and
  the receiving side prints both BEFORE the artefact is used. A hash that matches the previous
  delivery is the signal, and it is the only one that arrives on time.
  **CHECKABLE: yes** - assert every file named in a close's delivery list has a published md5
  beside it.

## FURTHER INSTANCE of R-S99-1 (an instrument bounded by the scheme it assumed)
R-S99-1 was written about a lateness measure bounded by the cron interval it was measuring. The
same shape appeared one session later in a different medium: an import resolver bounded by the
naming scheme its author assumed, reporting 3 edges where 84 existed. In both cases the
instrument returned its SMALLEST possible answer, and in both cases that answer was internally
consistent and passed every test written alongside it. The tell is the same both times - a
number that is not merely low but IMPOSSIBLE, here a test suite importing nothing at all.
**CHECKABLE: yes** - assert any published count of zero states what a non-zero result would
have looked like.
