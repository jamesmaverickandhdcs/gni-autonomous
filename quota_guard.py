...
# Constants -- GNI-R-111
# ============================================================
TPD_HARD_LIMIT   = 100_000   # Groq llama-3.3-70b hard daily limit
TPD_SAFE_CEILING =  85_000   # GNI-R-111: 85% safe ceiling
BUFFER           =  15_000   # Minimum buffer to maintain

# Token cost estimates per pipeline
PIPELINE_COSTS = {
    'gni_pipeline': 6_175,    # main analysis (CI disabled)
    'gni_mad':      12_393,   # MAD 15 calls
    'gni_verify':   0,        # zero Groq calls
    'gni_heartbeat':0,        # zero Groq calls -- GNI-R-114
    'gni_adaptive': 12_393,   # varies -- use MAD cost as max estimate
}
...
def get_today_usage(client) -> int:
    """
    Get total tokens used today from groq_daily_usage table.
    Returns total tokens used today across all pipelines.
    GNI-R-124: Never relies on Groq headers alone.
    """
    ...
    try:
        ...
        # If we cannot read the table, assume worst case -- do not block sacred runs
        # but alert admin
        return -1
...
def log_usage(client, pipeline: str, tokens_used: int,
    ...
    """
    Log token usage after a pipeline run.
    Called at the END of every Groq-consuming pipeline.
    GNI-R-124.
    """
    today = datetime.now(timezone.utc).date().isoformat()
    try:
...
def _send_telegram_alert(message: str):
    """Send Telegram alert when quota is critical."""
...
def check_quota(pipeline: str, sacred: bool = False) -> dict:
    """
    Main quota check. Call before any Groq-consuming pipeline run.

    Args:
        pipeline: pipeline name e.g. 'gni_pipeline', 'gni_mad', 'gni_adaptive'
        sacred:   True for 02:00+10:00 UTC sacred runs -- never blocked

    Returns dict:
        {
            'allowed':       True/False,
            'reason':        str,
            'tokens_used':   int (today so far),
            'tokens_remaining': int (before hard limit),
            'headroom':      int (before safe ceiling),
            'pipeline_cost': int (estimated cost of this run),
        }
    """
    client = _get_supabase_client()
    if not client:
        # Cannot check -- allow but warn
        return {
            'allowed': True,
    ...
    # Sacred runs (02:00+10:00 UTC gni_pipeline) -- NEVER block
    # GNI-R-121: Sacred runs always protected
    if sacred:
        result['allowed'] = True
        ...
        if tokens_used > TPD_SAFE_CEILING:
            alert = (
                ...
                'Pipeline: ' + pipeline + '\n'
                'Action: Sacred run proceeding but review token usage immediately.'
            )
            print('  ' + alert)
            _send_telegram_alert(alert)
        return result

    # Zero-cost pipelines -- always allow
    if pipeline_cost == 0:
        result['allowed'] = True
    ...
    # GNI-R-112: remaining must cover pipeline_cost + BUFFER
    ...
    # Warn if getting close (>75% of safe ceiling)
...
def print_status():
    """Print current quota status. For manual checks."""
    client = _get_supabase_client()
    if not client:
...
if __name__ == '__main__':
    # When run directly: print current quota status
    print_status()
